...
function openTestAdsWin() {
  ...
  interstitialAd.addEventListener('didReceiveAd', function (e) {
    Ti.API.info('interstitialAd - didReceiveAd: Did receive ad!');
    console.log(e);
  ...
  rewardedVideo.addEventListener('didFailToReceiveAd', function (error) {
  ...
  function loadOpenAd() {
    ...
    appOpenAd.addEventListener('didReceiveAd', function (e) {
      Ti.API.debug('appOpenAd - didReceiveAd: Did receive ad!');
      Ti.API.debug(e);
      reload_max_tries = 0;
      Titanium.App.Properties.setDouble('appOpenAdLoadTime', (new Date().getTime()));
    });
    appOpenAd.addEventListener('didFailToReceiveAd', function (error) {
      Ti.API.error('appOpenAd - Failed to load: ' + error.message);
      reloadAppOpenAd();
    });
    appOpenAd.addEventListener('didFailToShowAd', function (e) {
      Ti.API.error('appOpenAd - Failed to show: ' + e.message);
      reloadAppOpenAd();
    });
    ...
    appOpenAd.addEventListener('didRecordImpression', function (e) {
      Ti.API.debug('appOpenAd- didRecordImpression');
      Ti.API.debug(e);
    });
